function [] = SBL_SD(path_to_data, path_to_mask, b_vector, b_value, format_to_save_fODF, path_to_save_data, lambda1, lambda2, lambda_csf)
    ...
    for jj= 1:n2
        for kk = 1:n3
            if Mask(ii,jj,kk) > 0
                ...
                [Matrix_Dir_ijk, Fiso_ijk, Fw_ijk] = par_SBL(Signal_ijk(:), Kernel0, fODF0, Vrec, neig, threshold, Num_fmax);
...
function [Matrix_Dir_aux, fcsf, fw] = par_SBL(Signal, Kernel0, fODF0, Vrec, neig, threshold, Num_fmax)

% --- SBL estimation
fODF = deconv_iter_nnSBL_fast_SD (Signal(:), Kernel0, fODF0);
