function [] = LASSO_EBIC_SD(path_to_data, path_to_mask, b_vector, b_value, format_to_save_fODF, path_to_save_data, lambda1, lambda2, lambda_csf)
...
parfor ii=1:n1
    ...
    for jj= 1:n2
        for kk = 1:n3
            if (Mask(ii,jj,kk) > 0) && (sum(squeeze(E(ii,jj,kk,:)))>0)
                Signal_ijk = squeeze(E(ii,jj,kk,:));
                try
                    [Matrix_Dir_ijk, Fiso_ijk, Fw_ijk] = par_LASSO(Signal_ijk(:), Kernel0, Vrec, neig, threshold, Num_fmax);
                catch
                    Matrix_Dir_ijk = zeros(3*Num_fmax,1);
                    Fiso_ijk = 0;
                    Fw_ijk   = 0;
                end
                % ----------------------------------------
...
function [Matrix_Dir_aux, fcsf, fw] = par_LASSO(Signal, Kernel0, Vrec, neig, threshold, Num_fmax)

fODF = deconv_nnLASSO_BIC(Signal(:), Kernel0, Vrec, neig, threshold);
